...
model = "20240509_104247"
model_version = '0024999'
cfg = get_cfg()
...
class CCSN_GUI(MagicTemplate):
    ...
    @magicclass
    class Field_1():
        threshold_slider = field(FloatSlider, name = "Confidence Percentage",  options={"min": 0, "max": 1})
    ...
    def _image_select(self):
        ...
        '''
        if (
                re.search(r"^.+\.(?:(?:[tT][iI][fF][fF]?)|(?:[tT][iI][fF]))$", str(self.Field_2.image_selector.value)) == None
            ):
                raise ValueError("Image must be a tiff file")
        else:
            pass
            
        return str(self.Field_2.image_selector.value), tiff.imread(self.Field_2.image_selector.value)
