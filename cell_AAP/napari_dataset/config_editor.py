...
def open_extra_props_editor(widget) -> None:
    """
    Open an editable Python file containing template extra property functions.
    -------------------------------------------------------------------------------------------------------
    INPUTS:
		widget: QtWidgets.QWidget, parent widget used for save dialog and status
    OUTPUTS:
		None: None, writes file and opens it in the default editor
    """
    # Check if user already has an extra props file selected
    if widget.extra_props_file_path and os.path.exists(widget.extra_props_file_path):
        extra_props_file = widget.extra_props_file_path
        try:
            os.system(f"open {extra_props_file}")
            napari.utils.notifications.show_info(f"Opened existing extra props: {extra_props_file}")
            return
        except Exception as e:
            napari.utils.notifications.show_error(f"Error opening existing extra props: {str(e)}")
            return
    
    # Create default extra properties template
    default_extra_props = '''# Extra Properties Functions
# Add your custom property functions here
# Each function should take a region as input and return a scalar value
...
def custom_area_ratio(region):
    """
    Example custom property: ratio of area to bounding box area
    --------------------------------------------------------------------------------------------------------
    INPUTS:
        region: skimage.measure._regionprops.RegionProperties object
    OUTPUTS:
        ratio: float, ratio of region area to bounding box area
    """
    bbox_area = (region.bbox[2] - region.bbox[0]) * (region.bbox[3] - region.bbox[1])
    return region.area / bbox_area if bbox_area > 0 else 0.0

def custom_perimeter_ratio(region):
    """
    Example custom property: ratio of perimeter to area
    --------------------------------------------------------------------------------------------------------
    INPUTS:
        region: skimage.measure._regionprops.RegionProperties object
    OUTPUTS:
        ratio: float, ratio of perimeter to area
    """
    return region.perimeter / region.area if region.area > 0 else 0.0

# Add more custom functions as needed
'''
    
    # Get extra props file path
    extra_props_file, _ = QFileDialog.getSaveFileName(
        widget,
        "Save Extra Properties File",
        "",
        "Python Files (*.py);;All Files (*)"
    )
    
    if not extra_props_file:
        return
    
    try:
        # Save default extra properties
        with open(extra_props_file, 'w') as f:
            f.write(default_extra_props)
        
        # Update widget
        widget.update_extra_props_path(extra_props_file)
        
        # Open file in default editor
        os.system(f"open {extra_props_file}")
        
        napari.utils.notifications.show_info(f"Extra properties file created: {extra_props_file}")
        
    except Exception as e:
        napari.utils.notifications.show_error(f"Error creating extra properties file: {str(e)}")
